import cover from "../assets/coverPhotos/cover.jpg";
import { RiFootballLine } from "react-icons/ri";

const Login = () => {
  return (
    <div className="bg-gray-950 min-h-screen flex justify-center items-center font-sf">
      <div
        className="hidden md:flex md:w-1/2 h-screen bg-cover bg-center"
        style={{ backgroundImage: `url(${cover})` }}
      ></div>

      <div className="w-full min-h-screen md:w-1/2 flex flex-col justify-center items-center bg-gradient-to-br from-black to-gray-900 px-6 sm:px-10 py-10 md:py-0">
        
        <div className="flex items-center gap-2 mb-6">
          <RiFootballLine className="text-white text-4xl" />
          <h2 className="font-sf font-extrabold text-white text-3xl md:hidden">Jerseys for You</h2>
        </div>

        <div className="w-full max-w-md space-y-6">
          <h2 className="text-center text-white font-bold text-2xl sm:text-3xl">Sign into your account</h2>

          <div className="flex flex-col gap-2">
            <label htmlFor="email" className="text-white font-medium">Email</label>
            <input 
              type="email" 
              className="w-full bg-transparent border-2 rounded-md border-gray-600 text-gray-400 p-2 focus:border-white focus:outline-none" 
            />
          </div>

          <div className="flex flex-col gap-2">
            <div className="flex justify-between items-center">
              <label htmlFor="password" className="text-white font-medium">Password</label>
              <p className="text-gray-500 text-sm cursor-pointer hover:text-white transition">Forgot password?</p>
            </div>
            <input 
              type="password" 
              className="w-full bg-transparent border-2 rounded-md border-gray-600 text-gray-400 p-2 focus:border-white focus:outline-none" 
            />
          </div>

          <button className="w-full bg-white text-black py-2 rounded-md font-medium hover:bg-gray-200 transition-colors">
            Sign in
          </button>

          <div className="flex items-center gap-3">
            <div className="flex-grow h-px bg-gray-700"></div>
            <span className="text-gray-400 text-sm">Or continue with</span>
            <div className="flex-grow h-px bg-gray-700"></div>
          </div>

            <button className="w-full flex items-center justify-center gap-3 border border-gray-600 px-4 py-2 rounded-md hover:bg-gray-950 transition">
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 48 48">
              <path fill="#fbc02d" d="M43.611,20.083H42V20H24v8h11.303c-1.649,4.657-6.08,8-11.303,8c-6.627,0-12-5.373-12-12s5.373-12,12-12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C12.955,4,4,12.955,4,24s8.955,20,20,20s20-8.955,20-20C44,22.659,43.862,21.35,43.611,20.083z"></path>
              <path fill="#e53935" d="M6.306,14.691l6.571,4.819C14.655,15.108,18.961,12,24,12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C16.318,4,9.656,8.337,6.306,14.691z"></path>
              <path fill="#4caf50" d="M24,44c5.166,0,9.86-1.977,13.409-5.192l-6.19-5.238C29.211,35.091,26.715,36,24,36c-5.202,0-9.619-3.317-11.283-7.946l-6.522,5.025C9.505,39.556,16.227,44,24,44z"></path>
              <path fill="#1565c0" d="M43.611,20.083L43.595,20L42,20H24v8h11.303c-0.792,2.237-2.231,4.166-4.087,5.571c0.001-0.001,0.002-0.001,0.003-0.002l6.19,5.238C36.971,39.205,44,34,44,24C44,22.659,43.862,21.35,43.611,20.083z"></path>
            </svg>
            <span className="text-white font-medium">Sign in with Google</span>
          </button>
        </div>
      </div>
    </div>
  );
};

export default Login;
